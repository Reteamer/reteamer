    ...
    class PersonDeactivator
      class << self
        def deactivate(key, effective_date)
          update_subordinates_to_report_to_grand_boss(effective_date, key)
          deactivate_person_for_future(effective_date, key)
          deactivate_person(effective_date, key)
        end

        private

        def deactivate_person_for_future(effective_date, key)
