=begin
#ChartHop API

#REST API for ChartHop

OpenAPI spec version: V1.0.0
Contact: support@charthop.com
Generated by: https://github.com/swagger-api/swagger-codegen.git
Swagger Codegen version: 2.4.25

=end

require 'date'

module SwaggerClient
  class Comp
    # currency denomination of comp
    attr_accessor :currency

    # base annual salary
    attr_accessor :base

    # hourly compensation
    attr_accessor :hour

    # number of hours per week
    attr_accessor :hours

    # number of weeks per year
    attr_accessor :weeks

    # variable compensation, such as bonus or expected sales commissions
    attr_accessor :variable

    # variable compensation as a percentage of base annual
    attr_accessor :variable_pct

    # variable type - commission or bonus
    attr_accessor :variable_type

    # cost of compensation in benefits form
    attr_accessor :benefits

    # other compensation
    attr_accessor :other

    # annual expenses, such as T&E budget
    attr_accessor :expenses

    # planned stock grant, in shares
    attr_accessor :grant_shares

    # planned stock grant, in value (based on the stock's currency, not the comp)
    attr_accessor :grant_value

    # planned stock grant type
    attr_accessor :grant_type

    class EnumAttributeValidator
      attr_reader :datatype
      attr_reader :allowable_values

      def initialize(datatype, allowable_values)
        @allowable_values = allowable_values.map do |value|
          case datatype.to_s
          when /Integer/i
            value.to_i
          when /Float/i
            value.to_f
          else
            value
          end
        end
      end

      def valid?(value)
        !value || allowable_values.include?(value)
      end
    end

    # Attribute mapping from ruby-style variable name to JSON key.
    def self.attribute_map
      {
        :'currency' => :'currency',
        :'base' => :'base',
        :'hour' => :'hour',
        :'hours' => :'hours',
        :'weeks' => :'weeks',
        :'variable' => :'variable',
        :'variable_pct' => :'variablePct',
        :'variable_type' => :'variableType',
        :'benefits' => :'benefits',
        :'other' => :'other',
        :'expenses' => :'expenses',
        :'grant_shares' => :'grantShares',
        :'grant_value' => :'grantValue',
        :'grant_type' => :'grantType'
      }
    end

    # Attribute type mapping.
    def self.swagger_types
      {
        :'currency' => :'String',
        :'base' => :'Integer',
        :'hour' => :'Float',
        :'hours' => :'Integer',
        :'weeks' => :'Integer',
        :'variable' => :'Integer',
        :'variable_pct' => :'Float',
        :'variable_type' => :'String',
        :'benefits' => :'Integer',
        :'other' => :'Integer',
        :'expenses' => :'Integer',
        :'grant_shares' => :'Integer',
        :'grant_value' => :'Integer',
        :'grant_type' => :'String'
      }
    end

    # Initializes the object
    # @param [Hash] attributes Model attributes in the form of hash
    def initialize(attributes = {})
      return unless attributes.is_a?(Hash)

      # convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h| h[k.to_sym] = v }

      if attributes.has_key?(:'currency')
        self.currency = attributes[:'currency']
      end

      if attributes.has_key?(:'base')
        self.base = attributes[:'base']
      end

      if attributes.has_key?(:'hour')
        self.hour = attributes[:'hour']
      end

      if attributes.has_key?(:'hours')
        self.hours = attributes[:'hours']
      end

      if attributes.has_key?(:'weeks')
        self.weeks = attributes[:'weeks']
      end

      if attributes.has_key?(:'variable')
        self.variable = attributes[:'variable']
      end

      if attributes.has_key?(:'variablePct')
        self.variable_pct = attributes[:'variablePct']
      end

      if attributes.has_key?(:'variableType')
        self.variable_type = attributes[:'variableType']
      end

      if attributes.has_key?(:'benefits')
        self.benefits = attributes[:'benefits']
      end

      if attributes.has_key?(:'other')
        self.other = attributes[:'other']
      end

      if attributes.has_key?(:'expenses')
        self.expenses = attributes[:'expenses']
      end

      if attributes.has_key?(:'grantShares')
        self.grant_shares = attributes[:'grantShares']
      end

      if attributes.has_key?(:'grantValue')
        self.grant_value = attributes[:'grantValue']
      end

      if attributes.has_key?(:'grantType')
        self.grant_type = attributes[:'grantType']
      end
    end

    # Show invalid properties with the reasons. Usually used together with valid?
    # @return Array for valid properties with the reasons
    def list_invalid_properties
      invalid_properties = Array.new
      invalid_properties
    end

    # Check to see if the all the properties in the model are valid
    # @return true if the model is valid
    def valid?
      variable_type_validator = EnumAttributeValidator.new('String', ['BONUS', 'COMMISSION'])
      return false unless variable_type_validator.valid?(@variable_type)
      grant_type_validator = EnumAttributeValidator.new('String', ['NSO', 'ISO', 'RSU'])
      return false unless grant_type_validator.valid?(@grant_type)
      true
    end

    # Custom attribute writer method checking allowed values (enum).
    # @param [Object] variable_type Object to be assigned
    def variable_type=(variable_type)
      validator = EnumAttributeValidator.new('String', ['BONUS', 'COMMISSION'])
      unless validator.valid?(variable_type)
        fail ArgumentError, 'invalid value for "variable_type", must be one of #{validator.allowable_values}.'
      end
      @variable_type = variable_type
    end

    # Custom attribute writer method checking allowed values (enum).
    # @param [Object] grant_type Object to be assigned
    def grant_type=(grant_type)
      validator = EnumAttributeValidator.new('String', ['NSO', 'ISO', 'RSU'])
      unless validator.valid?(grant_type)
        fail ArgumentError, 'invalid value for "grant_type", must be one of #{validator.allowable_values}.'
      end
      @grant_type = grant_type
    end

    # Checks equality by comparing each attribute.
    # @param [Object] Object to be compared
    def ==(o)
      return true if self.equal?(o)
      self.class == o.class &&
          currency == o.currency &&
          base == o.base &&
          hour == o.hour &&
          hours == o.hours &&
          weeks == o.weeks &&
          variable == o.variable &&
          variable_pct == o.variable_pct &&
          variable_type == o.variable_type &&
          benefits == o.benefits &&
          other == o.other &&
          expenses == o.expenses &&
          grant_shares == o.grant_shares &&
          grant_value == o.grant_value &&
          grant_type == o.grant_type
    end

    # @see the `==` method
    # @param [Object] Object to be compared
    def eql?(o)
      self == o
    end

    # Calculates hash code according to all attributes.
    # @return [Fixnum] Hash code
    def hash
      [currency, base, hour, hours, weeks, variable, variable_pct, variable_type, benefits, other, expenses, grant_shares, grant_value, grant_type].hash
    end

    # Builds the object from hash
    # @param [Hash] attributes Model attributes in the form of hash
    # @return [Object] Returns the model itself
    def build_from_hash(attributes)
      return nil unless attributes.is_a?(Hash)
      self.class.swagger_types.each_pair do |key, type|
        if type =~ /\AArray<(.*)>/i
          # check to ensure the input is an array given that the attribute
          # is documented as an array but the input is not
          if attributes[self.class.attribute_map[key]].is_a?(Array)
            self.send("#{key}=", attributes[self.class.attribute_map[key]].map { |v| _deserialize($1, v) })
          end
        elsif !attributes[self.class.attribute_map[key]].nil?
          self.send("#{key}=", _deserialize(type, attributes[self.class.attribute_map[key]]))
        end # or else data not found in attributes(hash), not an issue as the data can be optional
      end

      self
    end

    # Deserializes the data based on type
    # @param string type Data type
    # @param string value Value to be deserialized
    # @return [Object] Deserialized data
    def _deserialize(type, value)
      case type.to_sym
      when :DateTime
        DateTime.parse(value)
      when :Date
        Date.parse(value)
      when :String
        value.to_s
      when :Integer
        value.to_i
      when :Float
        value.to_f
      when :BOOLEAN
        if value.to_s =~ /\A(true|t|yes|y|1)\z/i
          true
        else
          false
        end
      when :Object
        # generic object (usually a Hash), return directly
        value
      when /\AArray<(?<inner_type>.+)>\z/
        inner_type = Regexp.last_match[:inner_type]
        value.map { |v| _deserialize(inner_type, v) }
      when /\AHash<(?<k_type>.+?), (?<v_type>.+)>\z/
        k_type = Regexp.last_match[:k_type]
        v_type = Regexp.last_match[:v_type]
        {}.tap do |hash|
          value.each do |k, v|
            hash[_deserialize(k_type, k)] = _deserialize(v_type, v)
          end
        end
      else # model
        temp_model = SwaggerClient.const_get(type).new
        temp_model.build_from_hash(value)
      end
    end

    # Returns the string representation of the object
    # @return [String] String presentation of the object
    def to_s
      to_hash.to_s
    end

    # to_body is an alias to to_hash (backward compatibility)
    # @return [Hash] Returns the object in the form of hash
    def to_body
      to_hash
    end

    # Returns the object in the form of hash
    # @return [Hash] Returns the object in the form of hash
    def to_hash
      hash = {}
      self.class.attribute_map.each_pair do |attr, param|
        value = self.send(attr)
        next if value.nil?
        hash[param] = _to_hash(value)
      end
      hash
    end

    # Outputs non-array value in the form of hash
    # For object, use to_hash. Otherwise, just return the value
    # @param [Object] value Any valid value
    # @return [Hash] Returns the value in the form of hash
    def _to_hash(value)
      if value.is_a?(Array)
        value.compact.map { |v| _to_hash(v) }
      elsif value.is_a?(Hash)
        {}.tap do |hash|
          value.each { |k, v| hash[k] = _to_hash(v) }
        end
      elsif value.respond_to? :to_hash
        value.to_hash
      else
        value
      end
    end

  end
end
