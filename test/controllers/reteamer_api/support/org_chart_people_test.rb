...
class OrgChartPeopleTest < ActiveSupport::TestCase
  def setup
    ...
    @main_entry = ActsAsProposable.with_proposal(proposals(:main)) do
      Entry.create(effective_at: @today, versionable: People::Person.new(first_name: "Main Person"))
    end

    @proposed_entry = ActsAsProposable.with_proposal(proposals(:plan_b)) do
      Entry.create(effective_at: @today, versionable: People::Person.new(first_name: "Proposed Person"))
    end

    @other_entry = ActsAsProposable.with_proposal(proposals(:plan_c)) do
      Entry.create(effective_at: @today, versionable: People::Person.new(first_name: "Other Person"))
    end
  end
